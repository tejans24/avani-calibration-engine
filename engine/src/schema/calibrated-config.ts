import { z } from 'zod';
import { CorrectnessBar, Infra, RiskLevel, Runtime, Sensitivity, Topology } from './dials.js';
import { SCHEMA_VERSION, SEMVER, isSupportedVersion } from './version.js';

/**
 * calibrated-config — the output of the Calibration + Selection layers.
 *
 * The contract every downstream generator reads. `dials` is the controlled
 * vocabulary; `invariants` and `patterns` name the guarantees and stack pieces
 * selected from those dials.
 */

export const DialsSchema = z
  .object({
    correctness_bar: CorrectnessBar,
    sensitivity: Sensitivity,
    infra: Infra,
    runtime: Runtime,
    topology: Topology,
  })
  .describe('The resolved calibration dials for the app.');

export const RiskAssessmentSchema = z
  .object({
    feasibility: RiskLevel.describe('Overall feasibility rating.'),
    estimated_budget_usd: z.number().min(0).describe('Estimated token/build budget in USD.'),
    estimated_infra_monthly_usd: z.number().min(0).describe('Estimated monthly infrastructure cost in USD.'),
    timeline_risk: RiskLevel.describe('Risk that the timeline slips.'),
    mitigations: z.array(z.string().min(1)).describe('Recommended risk mitigations, e.g. start_with_mvp.'),
  })
  .describe('Feasibility, cost, and timeline assessment for the calibrated app.');

export const InfraOptionSchema = z
  .object({
    target: Infra,
    fit: z.enum(['recommended', 'viable', 'not-recommended']).describe('How this option stands for THIS intake on what the engine could evaluate.'),
    what: z.string().min(1).describe('What it is, in a line (SPEC §4.2 runtime shape + database).'),
    why: z.string().min(1).describe('Why it fits — or does not — for this intake, naming the inputs read.'),
    tradeoffs: z.string().min(1).describe('What you give up by choosing it.'),
    cost_shape: z.string().min(1).describe('Shape of the bill: flat, usage cliffs, architecture-dependent.'),
    ops_burden: z.enum(['lowest', 'low', 'medium', 'high', 'highest']).describe('Who runs it, and how much of them.'),
    becomes_the_answer_if: z.string().nullable().describe('The condition under which this option becomes the answer (SPEC §4.1 rule); null for the recommended one.'),
    evidence: z.enum(['shipped', 'house-position']).describe('Whether a running system backs this profile row (SPEC §4.2).'),
  })
  .describe('One option on the menu the owner decides from — every target is always listed and explained.');

export const InfraDecisionSchema = z
  .object({
    proposed: Infra.describe('The target the engine proposed.'),
    rule: z.number().int().min(1).describe('The SPEC §4.1 rule that fired.'),
    rationale: z.string().min(1).describe('Why that rule fired, in a sentence, with the inputs it read.'),
    runner_up: Infra.nullable().describe('The next-best admissible target — the alternative the owner is declining.'),
    unanswered: z.array(z.string().min(1)).describe('§4.1 inputs the intake profile could not answer; the owner weighs them before deciding.'),
    options: z.array(InfraOptionSchema).min(1).describe('The whole menu, recommended first: every target with its fit, trade-offs, cost shape, ops burden, and the condition that would make it the answer.'),
    status: z.enum(['proposed', 'decided']).describe('proposed = awaiting the owner; decided = the owner has accepted or overridden.'),
    decided: Infra.nullable().describe('The owner’s decision, once made. Equals `dials.infra` when decided.'),
    decided_by: z.enum(['owner']).nullable().describe('Only a human decides the deploy target. The engine proposes.'),
    note: z.string().nullable().describe('The owner’s reason, recorded for the decision log.'),
  })
  .describe('Deploy-target decision record: what the engine proposed, and what the owner decided (SPEC §4.1).');

export const DecisionsSchema = z
  .object({ infra: InfraDecisionSchema })
  .describe('Decisions the engine proposes but a human makes. Each carries the proposal and the decision side by side.');

export const CalibratedConfigSchema = z
  .object({
    schemaVersion: z.string().regex(SEMVER).describe('Schema version this document conforms to (semver).'),
    profile: z.string().min(1).describe('Profile name the calibration was run under, e.g. field-app.'),
    dials: DialsSchema,
    invariants: z.array(z.string().min(1)).describe('Named invariants to enforce (exact-match keys, e.g. observations_append_only_never_delete).'),
    patterns: z.array(z.string().min(1)).describe('Named stack patterns to inject, e.g. nextjs-app-router.'),
    risk_assessment: RiskAssessmentSchema,
    decisions: DecisionsSchema.optional(),
  })
  .describe('The calibrated configuration produced from an intake profile: dials, invariants, patterns, risk, and the propose/decide records.');

export type Dials = z.infer<typeof DialsSchema>;
export type InfraDecisionRecord = z.infer<typeof InfraDecisionSchema>;
export type InfraOptionRecord = z.infer<typeof InfraOptionSchema>;
export type RiskAssessment = z.infer<typeof RiskAssessmentSchema>;
export type CalibratedConfig = z.infer<typeof CalibratedConfigSchema>;

/** Parse and version-check a calibrated config. Throws on invalid or unsupported-major input. */
export function parseCalibratedConfig(data: unknown): CalibratedConfig {
  const parsed = CalibratedConfigSchema.parse(data);
  if (!isSupportedVersion(parsed.schemaVersion)) {
    throw new Error(
      `calibrated-config schemaVersion ${parsed.schemaVersion} is not supported by engine ${SCHEMA_VERSION} (major mismatch)`,
    );
  }
  return parsed;
}

export const safeParseCalibratedConfig = (data: unknown) => CalibratedConfigSchema.safeParse(data);
