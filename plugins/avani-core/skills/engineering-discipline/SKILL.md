...
These rules are language- and framework-agnostic by design — they hold for a TypeScript app, a Python service, or a server-rendered stack. Where a rule names a command, use the project's equivalent; the stack plugins and the project's own scripts supply the specifics.
...
| E2E (few) | critical flows, real stack | only third-party edges |

- Test behavior at the contract, not internals — refactors should not churn tests.
- Integration setup migrates a real, ephemeral database from empty using the project's migrate-deploy command — the migration path itself is continuously tested.
- Invariants test at the tier where they live (append-only at the DB tier, fuzzing at the service tier, accessibility at e2e).
- When test files run in parallel against **one shared database**, scope every fixture and assertion to data that test created (a per-test tenant/workspace row). Never wipe globally (an unscoped bulk delete) — it deletes a sibling test's data mid-run and the flake is unattributable.
- Test-runner transforms can mask module-system bugs: an import that crashes the real process on boot can pass under the runner. For any change touching process startup or adding a dependency, boot the real thing once.

## Mock policy

- Mock only boundaries you own. Wrap third parties (payments, auth, mail, …) in an adapter and mock the adapter.
...
- Outside-world effects (clocks, HTTP, mail, model calls) are **injectable, defaulting real**; tests inject fakes, and no test does live network I/O. Time-gated logic takes an injected clock (a `now` parameter) — test fixed instants, including DST boundaries.
